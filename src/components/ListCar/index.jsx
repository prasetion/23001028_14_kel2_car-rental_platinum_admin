    ...
    return (
        <div className="listcar-container">
            ...
            <div className="confirmation-container">
                <div className="confirmation-chart">
                    ...
                    <div className="confirm-button">
                        <button id="confirm-yes-button" onClick={() => handleDelete(selectedCar)}>YES</button>
                        <button id="confirm-no-button" onClick={() => setConfirmation(!confirmation)}>NO</button>
                    </div>
                </div>
            </div>
            )}
            ...
            <div className="category-button-containner">
                <button  id="category-button" className={activeButton === 1 ? 'active' : 'categoryButton'}
        disabled={activeButton === 1} onClick={() => {handleChangeFilter("", ""), handleButtonClick(1)}} >All</button>
                <button  id="category-button" className={activeButton === 2 ? 'active' : 'categoryButton'}
        disabled={activeButton === 2} onClick={() => {handleChangeFilter("", "small"), handleButtonClick(2)}}>2 - 4 people</button>
                <button  id="category-button" className={activeButton === 3 ? 'active' : 'categoryButton'}
        disabled={activeButton === 3} onClick={() => {handleChangeFilter("", "medium"), handleButtonClick(3)}}>4 - 6 people</button>
                <button  id="category-button" className={activeButton === 4 ? 'active' : 'categoryButton'}
        disabled={activeButton === 4} onClick={() => {handleChangeFilter("", "large"), handleButtonClick(4)}}>6 - 8 people</button>
            </div>
                ...
                {(cars.map((car) => (
                    <div key={car.id} id="carlist-card">
                        ...
                        <div>
                            ...
                            <p><span><img src={personIcon} alt="" /></span> {carCategory[car.category]}</p>
                            <p id="carlist-info"><span><img src={timeIcon} alt="" /></span> Update at {dayjs(car.updatedAt).format("D MMM YYYY, HH:mm")}</p>
                        </div>
                        <div className="carlist-button">
                            <button onClick={() => toggleConfirmation(car.id)} className="delete-button"><span></span>Delete</button>
